...
| 키 | 한도 | en-US | 38 비영어 |
|---|---|---|---|
| name | 30자 | 필수 (영문 ASO 키워드) | **필수 (각 언어 ASO 키워드)** |
| keywords | 100자 | 필수 (영문 검색어) | **필수 (각 언어 검색어)** |
...
→ ASO 우선 정책상 비영어 locale도 자기 언어로 검색 노출되어야 함.

### 정책 변경 이력

이전 정책 (폐기됨): "name/keywords는 en-US만 작성, 영어 ASO 독점."
- 문제: 한국어 사용자가 한국어로 App Store 검색해도 노출 안 됨.
- 마이너 시장 (니치 도메인) 타겟에서는 각 언어권 검색 노출이 필수.

현재 정책 (ASO-First): "모든 locale 이 자기 언어 ASO 키워드 보유."
- 한국어 사용자는 한국어로, 일본어 사용자는 일본어로 검색되어야 함.
- 각 locale 의 name + keywords 는 해당 언어권 검색량 기반 단어 사용.
...
| 행동 | 금지 이유 |
|---|---|
| 앱 브랜드명 영문 그대로 박기 (예: name 에 "Reptile Weather") | 검색 키워드 자리 낭비 |
| 직역 (예: name 을 "도마뱀 날씨" 로) | 검색량 0 단어 발생 |
| 의역 (예: name 을 "파충류 날씨" 로) | 검색 빈도 낮음 |

### 필수

각 locale 의 모든 ASO 키 (name, keywords, subtitle, description, promotional_text) 는:
1. 해당 언어권 사용자가 **실제로 검색하는 단어** 우선
2. 도메인 핵심 키워드 (영문 베이스에서 osk 가 제시) 를 각 언어로 ASO 매핑
3. 검색 키워드 4-7개를 자연스럽게 녹임

### 비교 예시 (한국어 — name + keywords)

```
[기존 정책 — 폐기됨]
"ko": {
  // name, keywords 키 자체 없음 (영어 독점)
  ...
}
→ 한국어 검색 시 노출 0
...
[현재 정책 — ASO-First]
"ko": {
  "name": "파충류 사육 - 테라리움 온습도",
  "keywords": "파충류,비어디드드래곤,볼파이톤,레오파드게코,테라리움,사육,UVB,온습도,탈피,일광욕",
  ...
}
→ 한국 사용자가 "파충류 사육", "테라리움", "UVB" 등으로 검색 시 노출
```
...
- 爬虫類飼育 (husbandry)
- ビバリウム (vivarium 음차)
- フトアゴ (bearded dragon 사육자 약칭)
- レオパ (leopard gecko 사육자 약칭)
- 温湿度 (humidity)
- 脱皮 (shed)
- UVB
- 飼育環境
...
[독일어 매핑]
- Reptilien Haltung (husbandry)
- Terrarium (terrarium 그대로)
...
### name (30자, 모든 locale 필수)

각 언어권 사용자가 App Store 에서 검색할 핵심 키워드 2-3개를 자연스럽게 조합.

❌ 직역: "도마뱀 날씨", "爬虫類の天気"
❌ 의역: "파충류 날씨", "爬虫類ウェザー"
❌ 영문 그대로: "Reptile Weather"
✅ ASO: "파충류 사육 - 테라리움 온습도" (3개 키워드)
✅ ASO: "爬虫類飼育 - テラリウム温湿度" (3개 키워드)

### keywords (100자, 모든 locale 필수)

콤마(,) 구분, **공백 없음**. 단어 10개 내외.
- 도메인 최상위 키워드 (예: 파충류, 爬虫類, Reptilien)
- 종 이름 (사육 커뮤니티 표준 음차/명칭)
- 도메인 도구/개념어 (UVB, 온습도, 테라리움 등)
- 행동 키워드 (사육, 탈피, 일광욕)

```
한국어: "파충류,비어디드드래곤,볼파이톤,레오파드게코,테라리움,사육,UVB,온습도,탈피,일광욕"
일본어: "爬虫類,フトアゴ,ボールパイソン,レオパ,テラリウム,飼育,UVB,温湿度,脱皮,日光浴"
독일어: "Reptilien,Bartagame,Königspython,Leopardgecko,Terrarium,Haltung,UVB,Luftfeuchte,Häutung,Sonnenbad"
```

### subtitle (30자, 모든 locale 필수)

name 과 다른 검색 키워드 2-3개 추가 노출. 중복 회피.

### promotional_text (170자, 모든 locale 필수)

ASO 키워드 자연스럽게 녹임 + 핵심 가치 제안.

### description (4000자, 모든 locale 필수)

ASO 키워드를 자연 문장 안에 반복 배치. EULA 링크 포함.
...
영문 베이스 (en-US) 에 위험 단어가 잔존하는 경우:
- en-US 는 그대로 유지 (영문 베이스 변경 금지)
- 비영어 locale 은 등가 안전 표현으로 우회 번역
...
- 도메인 키워드가 자연스럽게 녹았는지
- 글자수 한도 안에 들어왔는지
- name + keywords 가 각 언어 ASO 우선으로 작성됐는지
...
1. JSON 유효성
2. 39개 locale 키 일치
3. **모든 locale 이 8개 키 보유** (en-US 포함, 비영어 locale 모두)
4. 글자수 한도 (name 30, keywords 100, subtitle 30, promo 170, description 4000 등)
5. 영문 위험 단어 검출 (warning)
